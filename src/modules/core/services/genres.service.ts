...
class GenresService extends BaseService<Genre> {
  constructor(
    protected readonly genres: GenreRepository,
    protected readonly mapper: GenreMapper,
  ) {
    super(genres, mapper, 'Genres');
  }

  async onModuleInit() {
    this.logger.log(
      'Checking and seeding default genres on module initialization...',
    );

    const existingCount = await this.genres.count();
    if (existingCount > 0) {
      this.logger.log(`Found ${existingCount} existing genres → skipping seed`);
      return;
    }

    this.logger.log(
      'No genres found → seeding default genres for Album/Artist/Track',
    );
    for (const genre of defaultGenres) {
      const exists = await this.genres.findOne({
        where: { identify: genre.identify },
      });

      if (!exists) {
        await this.genres.create(genre);
        this.logger.log(`Created genre: ${genre.name} (${genre.identify})`);
      }
    }
